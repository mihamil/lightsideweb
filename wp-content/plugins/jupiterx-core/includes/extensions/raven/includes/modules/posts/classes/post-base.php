...
abstract class Post_Base extends Action_Base {
	...
	protected function get_render_categories() {
		...
		if ( ! in_array( $post_type, [ 'post', 'portfolio', 'product' ], true ) || ! $this->skin->get_instance_value( 'show_categories' ) ) {
			return;
		}
	...
	protected function render_excerpt() {
		...
		if ( 'product' === $post_type && class_exists( 'WooCommerce' ) ) {
			...
			if ( ! empty( $product_short_description_output ) ) {
				printf( '<div class="raven-post-excerpt">%1$s</div>', $product_short_description_output );
				return;
			}
		}
