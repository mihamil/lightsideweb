...
class Call_To_Action extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => '#111111',
				'selectors' => [
					'{{WRAPPER}} .raven-cta__button' => 'border-color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		echo sprintf( '<%1$s  %2$s>', Utils::validate_html_tag( $wrapper_tag ), $this->get_render_attribute_string( 'wrapper' ) );
		?>
		...
		<?php if ( $print_content ) : ?>
			<div class="raven-cta__content">
				...
				if ( ! empty( $settings['title'] ) ) :
					...
					echo sprintf( '<%1$s %2$s>%3$s</%1$s>', esc_html( $title_tag ), $this->get_render_attribute_string( 'title' ), esc_html( $settings['title'] ) );
				endif;
				?>
